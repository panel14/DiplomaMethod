...
/// <summary>
/// Unit tests for <see cref="RlsaWordSplitter"/>.
/// Saves 3× upscaled PNGs with green word boxes and red dividers for visual inspection.
/// </summary>
[TestFixture]
public class RlsaWordSplitterTests : BaseUnitTest
{
    ...
    private string ResultsPath
    {
        get
        {
            var raw    = "TestResults/RlsaWordSplitter";
            var asmDir = Path.GetDirectoryName(typeof(RlsaWordSplitterTests).Assembly.Location)
                         ?? Directory.GetCurrentDirectory();
            return Path.Combine(asmDir, raw);
        }
    }

    private readonly RlsaWordSplitter _splitter = new();
    ...
    public async Task Split_DetectsWords_AndSavesVisualization(string configKey)
    {
        ...
        string outFile = Path.Combine(ResultsPath, $"rlsa_{configKey}.png");
        ...
        string title)
    {
        ...
        canvas.DrawText($"RLSA — {title}   ({words.Count} word(s))", 6, HeaderH - 6,
